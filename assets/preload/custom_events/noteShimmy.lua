...
function onEvent(name, value1, value2)
    if name == 'noteShimmy' then
        ...
        --doTweenX('cammyMove', 'camHUD', (60 * getProperty('camHUD.zoom')) * val[(flippy and 1 or 2)], 0.2, 'circOut')
        for i = 0,7 do
            --noteTweenDirection("noteFlippy" ..i, i, 90 + (-5 * val[(flippy and 1 or 2)]), 0.2, 'circOut')
            --noteTweenAngle('noteangleyier'..i, i, 15 * val[((HUDPos > 0) and 1 or 2)], 0.1, 'circOut')
        end
        bounce = 0.25
        flippy = not flippy
    end
